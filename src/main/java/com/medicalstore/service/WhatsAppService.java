...
public class WhatsAppService {
    ...
    private String buildInvoiceMessage(Sale sale) {
        ...
        for (com.medicalstore.model.SaleItem item : sale.getItems()) {
            message.append("Medicine: ").append(item.getMedicine().getName()).append("\n");
            message.append("Qty: ").append(item.getQuantity()).append(" x ₹")
                    .append(String.format("%.2f", item.getUnitPrice())).append("\n");
            message.append("Total: ₹").append(String.format("%.2f", item.getTotalPrice())).append("\n");
            message.append("-----------------\n");
        }
