...
public class AnalyticsController {
        ...
        public String gstSummary(
                ...
                double annualTaxable = data.stream().mapToDouble(d -> ((Number) d.get("taxableAmount")).doubleValue())
                                .sum();
                ...
                switch (report) {
                        case "profit":
                                excelBytes = analyticsService
                                                .exportProfitPerMedicineExcel(
                                                                analyticsService.getProfitPerMedicine(start, end));
                                filename = "profit-per-medicine.xlsx";
                                break;
                        case "dead-stock":
                                excelBytes = analyticsService
                                                .exportDeadStockExcel(analyticsService
                                                                .getDeadStock(days != null ? days : 90));
                        ...
                        case "fast-moving":
                                ...
                                excelBytes = analyticsService.exportFastMovingExcel(
                                                analyticsService.getFastMovingItems(limit != null ? limit : 20, start,
                                                                end));
                                filename = "fast-moving-items.xlsx";
                                break;
                        case "gst":
                                excelBytes = analyticsService.exportGstSummaryExcel(
                                                analyticsService.getGstMonthlySummary(
